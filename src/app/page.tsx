import Image from "next/image";

export default function Home() {
  return (
    <main>
      <div className="mx-80 my-20">
        <section
          id="about"
          className="group/list flex flex-col gap-3 md:pt-4 lg:pt-8"
        >
          <span className="bg-[#20283A] p-8 rounded-2xl flex gap-8 items-center">
            <Image
              width={200}
              height={200}
              className="rounded-full h-full border-4 border-[#FACC15]"
              src="/daraja.jpg"
              alt="Image of Daraja Williams"
            />

            <div className="flex flex-col gap-2">
              <h2 className="text-center">Welcome!</h2>
              <p>
                I am a Full Stack Developer with expertise in various
                technologies. Feel free to explore my skills and projects
                through the navigation menu. As I continue to add new features
                to enhance this website, please feel free to send me your
                feedback!
              </p>
            </div>
          </span>
        </section>

        {/** Education Section */}
        <section
          id="education"
          className="flex flex-col gap-8 px-20 pb-10 mt-[112px] bg-[#20283A] rounded-2xl md:pt-4"
        >
          <h2 className="text-center">Education</h2>

          <span className="bg-[#0F172A] p-4 rounded-2xl">
            <h3 className="text-center">Algonquin College</h3>
            <p className="text-center">Computer Programming</p>
            <p className="text-center">2024 - 2026</p>
          </span>
          <span className="bg-[#0F172A] p-4 rounded-2xl">
            <h3 className="text-center">Brock University</h3>
            <p className="text-center">BSc Biotechnology (Honours)</p>
            <p className="text-center">2017 - 2023</p>
          </span>
        </section>

        {/** Skills Section */}
        <section
          id="skill"
          className="flex flex-col gap-8 px-20 pb-10 mt-[112px] bg-[#20283A] rounded-2xl md:pt-4"
        >
          <h2 className="text-center">Skills</h2>

          {/** Programming Skills*/}
          <span className="bg-[#0F172A] px-4 pt-4 pb-6 rounded-2xl">
            <h3 className="text-center mb-4">Programming</h3>

            <div className="flex flex-col gap-10 items-center justify-center md:flex-row md:gap-6 text-black">
              <span className="flex items-center justify-center px-2 py-1 rounded-xl bg-white gap-2">
                <svg className="fill-black w-10 h-10" viewBox="0 0 550 550">
                  <path d="M193.918 208.369c-4.729-10.456-6.849-22.652-3.236-33.731 3.612-11.327 11.703-20.413 19.794-28.878 22.525-22.531 50.285-39.085 72.316-61.986 12.197-12.573 22.278-27.634 25.762-44.937 2.864-12.695 1.496-25.764-1.117-38.337 11.7 13.319 15.559 32.363 12.197 49.541-3.608 19.292-14.316 36.344-26.886 51.031-10.081 11.827-21.659 22.282-33.731 31.993-14.065 11.327-27.88 23.524-36.716 39.457-7.472 12.943-9.215 28.876-4.11 42.942 8.341 24.146 27.756 42.071 38.338 64.848-11.703-10.332-23.152-21.036-33.86-32.361-11.198-11.95-21.78-24.644-28.751-39.582zm63.48-18.921c-2.115 19.792 8.213 38.462 20.539 53.151 5.972 6.596 11.076 14.687 11.323 23.899.251 12.318-6.716 23.774-15.684 31.861 2.119-.246 3.612-2.115 5.355-3.11 13.443-8.589 26.385-19.418 32.982-34.227 4.357-10.083 3.362-22.034-2.362-31.371-6.724-10.953-15.559-20.662-20.786-32.61-2.867-6.721-3.862-14.562-1.496-21.657 3.114-9.583 9.834-17.426 16.93-24.272 19.54-18.544 43.189-31.743 65.844-46.179-28.629 8.214-56.883 19.542-81.03 37.343-15.311 11.451-29.498 27.382-31.615 47.172zm136.049 93.604c13.568.748 26.882 10.704 29.374 24.397 2.366 11.825-3.358 23.524-10.705 32.485-12.075 14.438-28.382 24.771-44.807 33.609-1.622.991-2.99 2.237-4.235 3.608 21.659-5.478 43.314-13.689 60.867-27.756 9.705-8.091 18.294-18.799 20.163-31.619 1.743-11.076-2.86-22.528-11.077-29.871-9.96-9.09-24.021-12.448-37.218-10.704-7.593.995-15.931 2.613-21.158 8.961 6.226-1.241 12.32-3.733 18.796-3.11zM123.22 318.647c16.303 4.357 33.108 5.603 49.787 6.724 14.936.995 29.875 1.246 44.937 1.12 38.833-.619 77.916-3.236 116.003-11.699 3.608-.87 7.593-1.493 10.833-3.733 6.347-4.11 13.313-7.347 20.162-10.583-30.995 4.979-62.113 9.215-93.478 11.205-31.74 1.991-63.731 3.236-95.593 1.121-9.086-.87-18.423-1.371-26.886-4.858-1.994-.87-4.733-2.609-3.738-5.227 1.869-3.361 5.603-4.977 8.839-6.72 13.694-6.473 28.629-10.081 43.193-14.313-25.021-.376-49.916 5.971-72.814 15.806-5.105 2.491-10.83 4.481-14.936 8.714-1.622 1.739-1.622 4.732.247 6.222 3.735 3.361 8.711 4.854 13.444 6.221zm201.644 34.233c-21.784 3.859-43.694 7.472-65.726 8.589-24.147 1.618-48.294.247-72.191-2.241-4.604-.623-9.211-1.368-13.317-3.483-2.116-1.246-4.231-3.236-4.106-5.854.247-4.106 3.73-6.967 6.222-9.956-7.715 2.739-15.434 5.599-21.906 10.708-2.742 2.116-5.478 5.474-4.733 9.208 1.125 4.356 5.356 6.97 9.09 8.96 9.208 4.733 19.54 6.846 29.625 8.714 25.511 4.11 51.527 4.235 77.167 2.488 27.141-2.115 54.148-6.594 80.411-14.313-7.468-3.358-14.564-7.221-20.536-12.82zm-136.796 43.071c-6.97 1.99-14.066 4.357-19.79 8.957-2.868 2.241-5.105 6.104-3.734 9.713 1.743 4.604 6.1 7.347 10.203 9.705 10.708 5.854 22.78 8.589 34.604 10.708 26.765 4.229 54.27 3.608 80.908-1.12 15.806-2.989 31.615-7.221 46.301-13.815-9.584-3.984-18.917-8.467-27.878-13.693-15.559 2.738-31.246 5.603-47.178 6.598-21.032 1.618-42.319-.125-63.355-2.738-4.98-1.121-11.202-1.618-14.563-5.976-1.739-3.613 2.242-6.227 4.482-8.339zm170.277 80.031c17.424-3.604 34.977-7.719 50.908-15.806 4.976-2.867 11.076-5.979 12.698-11.95.87-5.725-5.105-8.714-9.337-11.08 2.613 2.993 4.356 7.347 1.74 10.83-4.357 5.853-11.821 8.091-18.42 10.332-20.66 5.85-42.072 8.216-63.355 10.582-56.385 5.102-113.146 6.348-169.528 1.618-18.92-1.994-38.217-4.109-56.264-10.829-2.86-1.246-7.217-2.492-7.217-6.352 1.117-3.354 4.357-5.227 7.217-6.845 12.945-6.595 27.384-10.207 41.822-11.077-4.228-2.491-9.208-2.738-14.062-2.613-12.076.373-23.9 3.483-35.349 7.347-9.834 3.604-19.916 7.59-27.76 14.811-3.111 2.735-5.971 7.962-2.739 11.699 4.98 5.353 12.699 6.72 19.54 8.338 38.338 6.599 77.171 10.328 116.011 11.699 51.531 1.498 103.434-.744 154.095-10.704zm51.033 6.724c-23.402 7.468-47.672 11.574-71.822 14.936-41.696 5.227-83.769 6.845-125.716 5.603-25.515-.995-51.03-2.738-76.176-6.974 5.85 3.984 13.071 5.227 19.794 7.096 28.257 5.976 57.255 7.096 86.01 7.966 42.19.748 84.387-.87 125.962-7.468 19.669-3.48 39.459-7.715 57.13-16.927 9.215-4.854 18.552-12.326 20.163-23.152-9.332 9.955-22.772 14.563-35.345 18.92z" />
                </svg>
                <h4>Java</h4>
              </span>

              <span className="flex items-center justify-center px-2 py-1 rounded-xl bg-white gap-2">
                <svg className="fill-black w-10 h-10" viewBox="0 0 50 50">
                  <path d="M25 2c-4.059 0-6.813.969-8.563 2.375C14.688 5.781 14 7.59 14 9.094V14h10v1H9.094c-1.828 0-3.684.793-5 2.469C2.777 19.145 2 21.645 2 25c0 3.355.777 5.855 2.094 7.531C5.41 34.207 7.266 35 9.094 35H14v5.906c0 1.504.688 3.313 2.438 4.719C18.188 47.031 20.94 48 25 48c4.059 0 6.813-.969 8.563-2.375C35.313 44.219 36 42.41 36 40.906V36H26v-1h14.906c1.828 0 3.684-.793 5-2.469C47.223 30.855 48 28.355 48 25c0-3.355-.777-5.855-2.094-7.531-1.316-1.676-3.172-2.469-5-2.469H36V9.094c0-1.543-.684-3.356-2.438-4.75C31.81 2.949 29.056 2 25 2Zm0 2c3.746 0 6.016.875 7.313 1.906C33.608 6.938 34 8.136 34 9.094V21a3 3 0 0 1-3 3H19c-2.059 0-3.832 1.27-4.594 3.063a4.804 4.804 0 0 0-.312.937A4.962 4.962 0 0 0 14 29v4H9.094c-1.27 0-2.446-.496-3.406-1.719C4.726 30.06 4 28.043 4 25c0-3.043.727-5.059 1.688-6.281C6.647 17.496 7.824 17 9.094 17H26v-5H16V9.094c0-.895.387-2.114 1.688-3.156C18.988 4.894 21.258 4 25 4Zm-5 3c-1.102 0-2 .898-2 2 0 1.102.898 2 2 2 1.102 0 2-.898 2-2 0-1.102-.898-2-2-2Zm16 10h4.906c1.27 0 2.446.496 3.407 1.719C45.273 19.94 46 21.957 46 25c0 3.043-.727 5.059-1.688 6.281-.96 1.223-2.136 1.719-3.406 1.719H24v5h10v2.906c0 .895-.387 2.114-1.688 3.157C31.012 45.105 28.742 46 25 46c-3.742 0-6.012-.895-7.313-1.938C16.387 43.02 16 41.801 16 40.907V29a3.01 3.01 0 0 1 3-3h12c2.746 0 5-2.254 5-5Zm-6 22c-1.102 0-2 .898-2 2 0 1.102.898 2 2 2 1.102 0 2-.898 2-2 0-1.102-.898-2-2-2Z" />
                </svg>

                <h4>Python</h4>
              </span>

              <span className="flex items-center justify-center px-2 py-1 rounded-xl bg-white gap-2">
                <svg
                  className="fill-black w-10 h-10"
                  viewBox="-4.8 -4.8 25.6 25.6"
                >
                  <path d="M0 1.75C0 .784.784 0 1.75 0h12.5C15.216 0 16 .784 16 1.75v2a.75.75 0 0 1-1.5 0v-2a.25.25 0 0 0-.25-.25H1.75a.25.25 0 0 0-.25.25v12.5c0 .138.112.25.25.25h13.5a.75.75 0 0 1 0 1.5H1.75A1.75 1.75 0 0 1 0 14.25V1.75Zm8.25 4A.75.75 0 0 1 9 6.5v4c0 1.005-.276 1.758-.845 2.232-.543.453-1.201.518-1.655.518a.75.75 0 0 1 0-1.5c.346 0 .563-.06.695-.17.106-.088.305-.335.305-1.08v-4a.75.75 0 0 1 .75-.75Zm3.026.83c.418-.498 1.075-.83 1.974-.83.835 0 1.465.286 1.89.735a.75.75 0 0 1-1.09 1.03c-.11-.116-.33-.265-.8-.265-.5 0-.72.168-.826.295A.767.767 0 0 0 12.25 8c0 .104.043.3.174.455.107.127.325.295.826.295.076 0 .149.011.218.032.663.094 1.164.39 1.506.798.394.47.526 1.024.526 1.42 0 .396-.132.95-.526 1.42-.418.498-1.075.83-1.974.83-.835 0-1.465-.286-1.89-.735a.75.75 0 0 1 1.09-1.03c.11.116.33.265.8.265.5 0 .72-.168.826-.295A.767.767 0 0 0 14 11c0-.104-.043-.3-.174-.455-.107-.127-.325-.295-.826-.295a.748.748 0 0 1-.218-.032c-.663-.095-1.164-.39-1.506-.798A2.263 2.263 0 0 1 10.75 8c0-.396.132-.95.526-1.42Z" />
                </svg>

                <h4>JavaScript</h4>
              </span>
            </div>
          </span>

          <span className="bg-[#0F172A] px-4 pt-4 pb-6 rounded-2xl">
            <h3 className="text-center mb-4">Web Development</h3>

            {/** WebDev Skills Row #1*/}
            <div className="flex flex-col gap-10 items-center justify-center md:flex-row md:gap-6 text-black mb-6">
              <span className="flex items-center justify-center px-2 py-1 rounded-xl bg-white gap-2">
                <svg
                  className="fill-black w-10 h-10"
                  viewBox="-5.4 -7 37.80 37.80"
                >
                  <path d="M15.902 11.974a2.41 2.41 0 1 1-4.82 0 2.41 2.41 0 0 1 4.82 0z" />
                  <path d="M13.491 17.494a27.278 27.278 0 0 1-8.749-1.206l.193.051a10.886 10.886 0 0 1-3.541-1.903l.021.016a3.414 3.414 0 0 1-1.41-2.467l-.001-.013c0-1.594 1.747-3.154 4.681-4.172 2.512-.827 5.403-1.304 8.405-1.304l.417.003h-.021a27.489 27.489 0 0 1 8.885 1.334l-.195-.055a11.058 11.058 0 0 1 3.433 1.814l-.024-.018a3.274 3.274 0 0 1 1.386 2.387l.001.013c0 1.656-1.954 3.332-5.103 4.374-2.363.729-5.08 1.149-7.895 1.149-.17 0-.339-.002-.508-.005h.025zm0-9.84c-.106-.002-.232-.002-.358-.002-2.881 0-5.656.452-8.259 1.289l.191-.053c-2.698.941-3.908 2.228-3.908 3.087 0 .893 1.301 2.3 4.153 3.274 2.249.697 4.834 1.099 7.513 1.099.235 0 .469-.003.702-.009l-.034.001a26.452 26.452 0 0 0 8.205-1.138l-.187.049c2.96-.984 4.32-2.391 4.32-3.28a2.247 2.247 0 0 0-.966-1.515l-.008-.005a9.806 9.806 0 0 0-2.97-1.574l-.07-.02c-2.368-.766-5.093-1.208-7.92-1.208-.142 0-.283.001-.424.003h.021z" />
                  <path d="m8.023 23.986-.053.001c-.453 0-.878-.119-1.245-.327l.013.007c-1.378-.8-1.858-3.092-1.28-6.141.697-3.128 1.827-5.894 3.344-8.4l-.069.124a27.856 27.856 0 0 1 5.425-6.872l.02-.018A10.943 10.943 0 0 1 17.365.332l.073-.027c.407-.196.885-.31 1.39-.31.501 0 .976.113 1.4.314l-.02-.008c1.435.826 1.911 3.36 1.238 6.606-.72 2.967-1.818 5.58-3.262 7.959l.07-.123c-1.452 2.63-3.209 4.882-5.266 6.819l-.014.013a10.872 10.872 0 0 1-3.321 2.082l-.074.026c-.46.181-.992.292-1.549.305h-.006zM9.228 9.539l.499.288c-1.381 2.28-2.465 4.926-3.106 7.74l-.034.18c-.533 2.809-.019 4.498.72 4.926.191.102.417.161.657.161l.061-.001h-.003c.955 0 2.458-.605 4.196-2.122a26.398 26.398 0 0 0 4.972-6.404l.068-.135a26.071 26.071 0 0 0 3.025-7.307l.038-.182c.629-3.058.086-4.93-.686-5.378a2.246 2.246 0 0 0-1.814.092l.013-.006a9.858 9.858 0 0 0-2.907 1.846l.007-.006a26.608 26.608 0 0 0-5.136 6.468l-.069.132z" />
                  <path d="M18.96 24c-1.306 0-2.96-.787-4.69-2.276a27.794 27.794 0 0 1-5.464-6.834l-.072-.137c-1.43-2.349-2.551-5.075-3.215-7.975l-.036-.185a10.75 10.75 0 0 1-.255-2.357c0-.527.037-1.044.108-1.551l-.007.058A3.271 3.271 0 0 1 6.701.344l.01-.007c1.43-.83 3.865.024 6.342 2.228a27.865 27.865 0 0 1 5.118 6.54l.072.138a26.769 26.769 0 0 1 3.242 7.798l.038.186c.163.709.257 1.524.257 2.36 0 .577-.044 1.143-.13 1.696l.008-.062a3.419 3.419 0 0 1-1.429 2.456l-.011.007a2.435 2.435 0 0 1-1.207.316l-.054-.001h.003zm-9.228-9.823a27.066 27.066 0 0 0 5.271 6.656l.02.018c2.166 1.863 3.884 2.266 4.628 1.834.773-.446 1.339-2.276.754-5.233-.702-2.89-1.787-5.434-3.219-7.741l.065.112a26.77 26.77 0 0 0-4.948-6.384l-.012-.011C9.957 1.354 8.066.888 7.293 1.334a2.234 2.234 0 0 0-.825 1.593v.007a9.854 9.854 0 0 0 .162 3.499l-.013-.066c.673 2.952 1.747 5.559 3.182 7.924l-.066-.117z" />
                </svg>

                <h4>React</h4>
              </span>

              <span className="flex items-center justify-center px-2 py-1 rounded-xl bg-white gap-2">
                <svg className="fill-black w-10 h-10" viewBox="0 0 32 32">
                  <path d="M23.749 30.005c-.119.063-.109.083.005.025a.31.31 0 0 0 .095-.061c0-.021 0-.021-.1.036zm.24-.13c-.057.047-.057.047.011.016a.249.249 0 0 0 .068-.047c0-.027-.016-.021-.079.031zm.156-.094c-.057.047-.057.047.011.016a.246.246 0 0 0 .068-.048c0-.025-.016-.02-.079.032zm.158-.093c-.057.047-.057.047.009.015.037-.02.068-.041.068-.047 0-.025-.016-.02-.077.032zm.213-.141c-.109.073-.147.12-.047.068.067-.041.181-.131.161-.131-.043.016-.079.043-.115.063zM14.953.011c-.073.005-.292.025-.484.041-4.548.412-8.803 2.86-11.5 6.631a15.828 15.828 0 0 0-2.824 6.989c-.129.88-.145 1.14-.145 2.333 0 1.192.016 1.448.145 2.328.871 6.011 5.147 11.057 10.943 12.927 1.043.333 2.136.563 3.381.704.484.052 2.577.052 3.061 0 2.152-.24 3.969-.771 5.767-1.688.276-.14.328-.177.291-.208a340.89 340.89 0 0 1-2.609-3.495l-2.557-3.453-3.203-4.745a416.396 416.396 0 0 0-3.229-4.744c-.011 0-.025 2.109-.031 4.681-.011 4.505-.011 4.688-.068 4.792a.572.572 0 0 1-.276.287c-.099.047-.188.057-.661.057h-.541l-.141-.088a.595.595 0 0 1-.208-.229l-.068-.141.005-6.271.011-6.271.099-.125a.753.753 0 0 1 .229-.187c.131-.063.183-.073.724-.073.635 0 .74.025.907.208a602.855 602.855 0 0 1 3.859 5.812c2.079 3.152 4.917 7.453 6.312 9.563l2.537 3.839.125-.083a16.346 16.346 0 0 0 3.285-2.885 15.935 15.935 0 0 0 3.767-8.177c.129-.88.145-1.141.145-2.333 0-1.193-.016-1.448-.145-2.328C30.985 7.668 26.709 2.622 20.913.751a16.983 16.983 0 0 0-3.328-.697c-.303-.031-2.371-.068-2.631-.041zM21.5 9.688a.623.623 0 0 1 .317.364c.027.084.032 1.823.027 5.74l-.011 5.624-.989-1.52-.995-1.521v-4.083c0-2.647.011-4.131.025-4.204a.67.67 0 0 1 .313-.395c.124-.063.172-.068.667-.068.463 0 .541.005.645.063z" />
                </svg>

                <h4>Next.js</h4>
              </span>

              <span className="flex items-center justify-center px-2 py-1 rounded-xl bg-white gap-2">
                <svg className="fill-black w-10 h-10" viewBox="0 0 24 24">
                  <path d="M12 6.036c-2.667 0-4.333 1.325-5 3.976 1-1.325 2.167-1.822 3.5-1.491.761.189 1.305.738 1.906 1.345C13.387 10.855 14.522 12 17 12c2.667 0 4.333-1.325 5-3.976-1 1.325-2.166 1.822-3.5 1.491-.761-.189-1.305-.738-1.907-1.345-.98-.99-2.114-2.134-4.593-2.134zM7 12c-2.667 0-4.333 1.325-5 3.976 1-1.326 2.167-1.822 3.5-1.491.761.189 1.305.738 1.907 1.345.98.989 2.115 2.134 4.594 2.134 2.667 0 4.333-1.325 5-3.976-1 1.325-2.167 1.822-3.5 1.491-.761-.189-1.305-.738-1.906-1.345C10.613 13.145 9.478 12 7 12z" />
                </svg>

                <h4>Tailwind</h4>
              </span>
            </div>

            {/** WebDev Skills Row #2*/}
            <div className="flex flex-col gap-10 items-center justify-center md:flex-row md:gap-6 text-black">
              <span className="flex items-center justify-center px-2 py-1 rounded-xl bg-white gap-2">
                <svg
                  className="fill-black w-10 h-10"
                  viewBox="-2.4 -3.8 28.80 28.80"
                >
                  <path className="fill-white" d="M0 0h24v24H0z" />
                  <path d="m12 18.178-4.62-1.256-.328-3.544h2.27l.158 1.844 2.52.667 2.52-.667.26-2.866H6.96l-.635-6.678h11.35l-.227 2.21H8.822l.204 2.256h8.217l-.624 6.778L12 18.178zM3 2h18l-1.623 18L12 22l-7.377-2L3 2zm2.188 2L6.49 18.434 12 19.928l5.51-1.494L18.812 4H5.188z" />
                </svg>

                <h4>HTML</h4>
              </span>

              <span className="flex items-center justify-center px-2 py-1 rounded-xl bg-white gap-2">
                <svg
                  className="fill-black w-10 h-10"
                  viewBox="-2.4 -1.5 20.8 20.8"
                >
                  <path d="M7.198 3.06a2.883 2.883 0 0 0-2.886 2.88v6.407A2.155 2.155 0 0 1 2.156 14.5 2.155 2.155 0 0 1 0 12.347a.78.78 0 0 1 1.563 0 .593.593 0 0 0 1.187 0V5.941A4.444 4.444 0 0 1 7.198 1.5a4.449 4.449 0 0 1 4.274 3.206A4.673 4.673 0 0 1 16 9.373a.785.785 0 0 1-.008.114.784.784 0 0 1 .008.115v4.118c0 .43-.35.78-.781.78H7.198a.78.78 0 0 1-.781-.78V9.602c0-.431.35-.78.78-.78a2.883 2.883 0 0 0 2.886-2.881A2.883 2.883 0 0 0 7.198 3.06Zm7.248 6.427a.784.784 0 0 1-.008-.114 3.112 3.112 0 0 0-2.805-3.095 4.446 4.446 0 0 1-3.654 4.035v2.627h2.448v-1.737a.78.78 0 0 1 1.563 0v1.737h2.447V9.602c0-.04.003-.077.009-.115Z" />
                </svg>

                <h4>PHP</h4>
              </span>

              <span className="flex items-center justify-center px-2 py-1 rounded-xl bg-white gap-2">
                <svg
                  className="fill-black w-10 h-10"
                  viewBox="-2.4 -3.8 28.80 28.80"
                >
                  <path d="m2.877 1.992 1.967 17.885 7.148 2.19.293-.09 6.856-2.1 1.982-17.885H2.877zm2.23 2H18.89L17.297 18.35l-5.305 1.625-5.305-1.625-1.58-14.358zm2.112 2v2h7.437l-.232 2h-3.432v2h3.201l-.277 2.399-1.924.56-1.912-.56-.135-1.118-.119-.992-1.986.239.119.992.291 2.43 3.742 1.093 3.756-1.094 1.154-9.949H7.22z" />
                </svg>

                <h4>CSS</h4>
              </span>
            </div>
          </span>

          {/** Database Skills*/}
          <span className="bg-[#0F172A] px-4 pt-4 pb-6 rounded-2xl">
            <h3 className="text-center mb-4">Database Management</h3>

            <div className="flex flex-col gap-10 items-center justify-center md:flex-row md:gap-6 text-black">
              <span className="flex items-center justify-center px-2 py-1 rounded-xl bg-white gap-2">
                <svg
                  className="fill-black w-10 h-10"
                  viewBox="-4.8 -4.8 33.6 33.6"
                >
                  <path d="m24.129 23.412-.508-.484a6.66 6.66 0 0 0-.809-.891l-.005-.004q-.448-.407-.931-.774-.387-.266-1.064-.641a1.6 1.6 0 0 1-.818-.824l-.004-.01-.048-.024c.212-.021.406-.06.592-.115l-.023.006.57-.157c.236-.074.509-.122.792-.133h.006c.298-.012.579-.06.847-.139l-.025.006q.194-.048.399-.109t.351-.109v-.169q-.145-.217-.351-.496a2.561 2.561 0 0 0-.443-.468l-.005-.004q-.629-.556-1.303-1.076a15.783 15.783 0 0 0-1.311-.916l-.068-.04a4.983 4.983 0 0 0-.825-.435l-.034-.012q-.448-.182-.883-.399a1.638 1.638 0 0 0-.327-.119l-.011-.002a.516.516 0 0 1-.29-.169l-.001-.001a2.999 2.999 0 0 1-.355-.609l-.008-.02q-.145-.339-.314-.651-.363-.702-.702-1.427t-.651-1.452q-.217-.484-.399-.967a5.395 5.395 0 0 0-.461-.942l.013.023a17.366 17.366 0 0 0-1.331-1.961l.028.038a14.57 14.57 0 0 0-1.459-1.59l-.008-.007a17.619 17.619 0 0 0-1.632-1.356l-.049-.035q-.896-.651-1.96-1.282a3.7 3.7 0 0 0-.965-.393l-.026-.006-1.113-.278-.629-.048q-.314-.024-.629-.024a1.68 1.68 0 0 1-.387-.279 3.581 3.581 0 0 0-.353-.295l-.01-.007a11.686 11.686 0 0 0-2.043-.93L2.071.18A1.355 1.355 0 0 0 .9.096L.909.093a1.36 1.36 0 0 0-.795.84l-.003.01a1.515 1.515 0 0 0 .232 1.549l-.002-.003q.544.725.834 1.14.217.291.448.605c.141.188.266.403.367.63l.008.021c.056.119.105.261.141.407l.003.016q.048.206.121.448.217.556.411 1.14c.141.425.297.785.478 1.128l-.019-.04q.145.266.291.52T3.738 9a.868.868 0 0 0 .241.242l.003.002a.406.406 0 0 1 .169.313v.001a1.326 1.326 0 0 0-.217.586l-.001.006a4.227 4.227 0 0 1-.153.695l.008-.03a7.103 7.103 0 0 0-.351 2.231c0 .258.013.512.04.763l-.003-.031c.06.958.349 1.838.812 2.6l-.014-.025c.197.295.408.552.641.787a.914.914 0 0 0 1.106.203l-.005.002a.926.926 0 0 0 .617-.827v-.002c.048-.474.12-.898.219-1.312l-.013.067a.595.595 0 0 0 .038-.211l-.002-.045v.002q-.012-.109.133-.206v.048q.145.339.302.677t.326.677c.295.449.608.841.952 1.202l-.003-.003a7.74 7.74 0 0 0 1.127 1.001l.022.015c.212.162.398.337.566.528l.004.004c.158.186.347.339.56.454l.01.005v-.024h.048a.461.461 0 0 0-.18-.205l-.002-.001a1.829 1.829 0 0 1-.211-.136l.005.003q-.217-.217-.448-.484t-.423-.508q-.508-.702-.969-1.467t-.871-1.555q-.194-.387-.375-.798t-.351-.798a.997.997 0 0 1-.096-.334v-.005a.318.318 0 0 0-.168-.265l-.002-.001a2.947 2.947 0 0 1-.408.545l.001-.001a1.974 1.974 0 0 0-.382.58l-.005.013a4.272 4.272 0 0 0-.289 1.154l-.002.019q-.072.641-.145 1.318l-.048.024-.024.024a.855.855 0 0 1-.59-.443l-.002-.005q-.182-.351-.326-.69a6.448 6.448 0 0 1-.423-2.144v-.009a6.218 6.218 0 0 1 .286-2.318l-.012.044q.072-.266.314-.896t.097-.871a.876.876 0 0 0-.265-.41l-.001-.001a3.31 3.31 0 0 1-.335-.335l-.003-.004q-.169-.244-.326-.52t-.278-.544a11.408 11.408 0 0 1-.474-1.353l-.022-.089a10.174 10.174 0 0 0-.546-1.503l.026.064a3.32 3.32 0 0 0-.39-.669l.006.008q-.244-.326-.436-.617-.244-.314-.484-.605a3.414 3.414 0 0 1-.426-.657l-.009-.02a1.638 1.638 0 0 1-.119-.327l-.002-.011a.406.406 0 0 1 .049-.34l-.001.002a.303.303 0 0 1 .073-.145.308.308 0 0 1 .143-.072h.002a.55.55 0 0 1 .536-.035l-.003-.001c.219.062.396.124.569.195l-.036-.013q.459.194.847.375c.298.142.552.292.792.459l-.018-.012q.194.121.387.266t.411.291h.339q.387 0 .822.037c.293.023.564.078.822.164l-.024-.007c.481.143.894.312 1.286.515l-.041-.019q.593.302 1.125.641c.589.367 1.098.743 1.577 1.154l-.017-.014c.5.428.954.867 1.38 1.331l.01.012c.416.454.813.947 1.176 1.464l.031.047c.334.472.671 1.018.974 1.584l.042.085a4.6 4.6 0 0 1 .234.536l.011.033q.097.278.217.57.266.605.57 1.221t.57 1.198l.532 1.161c.187.406.396.756.639 1.079l-.011-.015c.203.217.474.369.778.422l.008.001c.368.092.678.196.978.319l-.047-.017c.143.065.327.134.516.195l.04.011c.212.065.396.151.565.259l-.009-.005c.327.183.604.363.868.559l-.021-.015q.411.302.822.57.194.145.651.423t.484.52a11.202 11.202 0 0 0-1.834.087l.056-.006a5.959 5.959 0 0 0-1.479.39l.04-.014a2.556 2.556 0 0 1-.388.129l-.019.004a.312.312 0 0 0-.266.277v.001c.061.076.11.164.143.26l.002.006c.034.102.075.19.125.272l-.003-.006c.119.211.247.393.391.561l-.004-.005c.141.174.3.325.476.454l.007.005q.244.194.508.399c.161.126.343.25.532.362l.024.013c.284.174.614.34.958.479l.046.016c.374.15.695.324.993.531l-.016-.011q.291.169.58.375t.556.399c.073.072.137.152.191.239l.003.005a.573.573 0 0 0 .36.193h.003v-.048a.516.516 0 0 1-.184-.267l-.001-.004a.95.95 0 0 0-.112-.273l.002.004zM5.553 4.207q-.194 0-.363.012a1.285 1.285 0 0 0-.323.063l.009-.003v.024h.048q.097.145.244.326t.266.351l.387.798.048-.024a.735.735 0 0 0 .252-.321l.002-.005c.052-.139.082-.301.082-.469l-.001-.054v.003a.504.504 0 0 1-.108-.154l-.001-.003-.081-.182a.52.52 0 0 0-.214-.192l-.003-.001a.93.93 0 0 1-.244-.169z" />
                </svg>

                <h4>MySQL</h4>
              </span>

              <span className="flex items-center justify-center px-2 py-1 rounded-xl bg-white gap-2">
                <svg className="fill-black w-10 h-10" viewBox="-4 -4 40 40">
                  <path d="M21.272 22.141H10.734a6.143 6.143 0 0 1-.006-12.282h10.556a6.141 6.141 0 0 1 0 12.282h-.012.001zm.241-15.623H10.491A9.481 9.481 0 0 0 1.017 16a9.482 9.482 0 0 0 9.463 9.482h11.033c5.231-.008 9.469-4.25 9.469-9.482s-4.238-9.474-9.468-9.482h-.001z" />
                </svg>

                <h4>Oracle</h4>
              </span>

              <span className="flex items-center justify-center px-2 py-1 rounded-xl bg-white gap-2">
                <svg
                  className="fill-black w-10 h-10"
                  viewBox="-3.2 -5 38.40 38.40"
                >
                  <path d="M24.295 9.929a.56.56 0 0 1-.191.387l-.001.001a.894.894 0 0 1-.504.278l-.005.001a.68.68 0 0 1-.094.007h-.004a.752.752 0 0 1-.694-.46l-.002-.005c-.03-.221.33-.388.701-.44s.764.011.794.231zm-9.91.514a.822.822 0 0 1-.766.528h-.011.001a.91.91 0 0 1-.105-.007h.004a.997.997 0 0 1-.63-.392l-.002-.003a.539.539 0 0 1-.132-.354v-.001a.278.278 0 0 1 .115-.185l.001-.001a1.094 1.094 0 0 1 .77-.107l-.007-.001c.395.055.803.242.764.523zm11.018 7.643-.107-.134-.044-.055a6.164 6.164 0 0 0 .725-2.921c0-.488-.056-.962-.162-1.418l.008.042a10.08 10.08 0 0 1-.111-1.508v-.021.001a10.9 10.9 0 0 1 .158-1.429l-.01.066A8.35 8.35 0 0 0 26 9.165l-.001-.151v.008a.686.686 0 0 0 .019-.162c0-.027-.002-.053-.004-.079v.003a9.49 9.49 0 0 0-2.168-4.073l.007.008a9.795 9.795 0 0 0-3.3-2.523l-.06-.025a11.255 11.255 0 0 1 2.316-.234l.225.002h-.011l.15-.002a7.466 7.466 0 0 1 5.866 2.838l.012.016a.962.962 0 0 1 .08.12l.003.005c.904 1.694-.345 7.842-3.732 13.172zm-.286-8.764a9.111 9.111 0 0 1-.14 1.307l.008-.055c-.078.425-.134.931-.157 1.445l-.001.025v.056c0 .567.042 1.124.124 1.668l-.008-.061c.085.377.134.809.134 1.254 0 .763-.144 1.493-.407 2.162l.014-.04a4.07 4.07 0 0 1-.224-.453l-.011-.029a9.727 9.727 0 0 0-.406-.77c-.769-1.38-2.571-4.611-1.649-5.929.474-.678 1.676-.707 2.722-.579zm-.711 11.585c-.051-1.039.336-1.148.746-1.263q.085-.023.169-.051c.05.044.105.087.162.125l.005.003c.62.273 1.342.431 2.102.431.592 0 1.161-.096 1.693-.274l-.038.011a5.005 5.005 0 0 1-1.16.738l-.031.013a6.31 6.31 0 0 1-2.173.454h-.01a2.734 2.734 0 0 1-1.482-.195l.018.007zm-1.113 2.026c-.021.221-.045.47-.077.745l-.182.548a.437.437 0 0 0-.023.134l.001.089c0 .355-.053.699-.151 1.022l.006-.025c-.116.389-.196.84-.223 1.305l-.001.016a3.307 3.307 0 0 1-3.008 3.194l-.013.001c-1.894.406-2.23-.621-2.526-1.527q-.045-.142-.096-.283a9.25 9.25 0 0 1-.252-2.171c0-.36.02-.715.059-1.065l-.004.043a10.001 10.001 0 0 0-.432-3.378l.019.071q.008-.55.024-1.114v-.012a.444.444 0 0 0-.02-.132l.001.003a1.872 1.872 0 0 0-.058-.273l.003.013a1.806 1.806 0 0 0-.964-1.164l-.011-.005a1.358 1.358 0 0 0-.907-.107l.009-.002c.128-.47.26-.854.412-1.228l-.026.073.066-.177c.074-.2.167-.407.266-.626a8.827 8.827 0 0 0 .865-3.841c0-.938-.144-1.842-.411-2.692l.017.063a2.314 2.314 0 0 0-2.805-1.883l.016-.003a6.412 6.412 0 0 0-2.171.682l.035-.017q-.124.063-.245.13a9.286 9.286 0 0 1 2.181-5.615l-.012.014c.118-.119.242-.232.37-.338l.009-.007a.45.45 0 0 0 .182-.081l-.001.001a5.455 5.455 0 0 1 3.515-1.04l-.013-.001c.526.008 1.034.044 1.534.108l-.067-.007a8.618 8.618 0 0 1 5.032 2.963l.011.014a9.864 9.864 0 0 1 1.766 2.998l.022.069a3.399 3.399 0 0 0-3.346.972l-.002.002c-1.24 1.773.679 5.215 1.601 6.869.169.303.315.565.361.676.26.601.587 1.118.98 1.577l-.007-.008c.087.109.171.214.236.306-.501.144-1.401.478-1.319 2.146-.015.195-.053.558-.104 1.018a7.887 7.887 0 0 0-.123.93l-.002.028zm-9.202-5.714-.066.176c-.137.328-.279.745-.397 1.172l-.019.081a3.147 3.147 0 0 1-2.261-1.001l-.002-.002a3.716 3.716 0 0 1-.975-3.146l-.003.021c.097-.72.153-1.551.153-2.396 0-.502-.02-.999-.058-1.491l.004.065c-.006-.107-.012-.201-.015-.275a4.838 4.838 0 0 1 3.32-.965l-.017-.001c.554.129.971.588 1.037 1.153l.001.006c.238.728.375 1.566.375 2.435a8.07 8.07 0 0 1-.809 3.532l.021-.048c-.105.233-.204.453-.289.682zm-2.617 4.984a2 2 0 0 1-.549-.227l.009.005c.172-.073.375-.134.585-.173l.019-.003c1.604-.33 1.851-.563 2.392-1.25.124-.157.264-.336.459-.553a.44.44 0 0 0 .091-.159l.001-.003c.213-.189.34-.137.546-.052a.942.942 0 0 1 .46.587l.001.007a.654.654 0 0 1-.058.558l.002-.003a3.507 3.507 0 0 1-3.983 1.258l.025.007zm-7.507-6.357c-.651-1.985-1.181-4.34-1.494-6.764l-.021-.199a5.87 5.87 0 0 1-.095-1.071c0-1.806.789-3.427 2.041-4.537l.006-.005c2.295-1.623 6.048-.676 7.633-.163l-.012.012a10.904 10.904 0 0 0-2.463 7.198v-.013c0 .103.008.249.02.449a18.133 18.133 0 0 1-.102 3.738l.01-.091a4.593 4.593 0 0 0 1.218 3.863l-.002-.002q.151.158.315.297a64.838 64.838 0 0 0-2.377 2.696c-.709.853-1.199.689-1.36.636a3.505 3.505 0 0 1-1.538-1.631l-.009-.022a21.48 21.48 0 0 1-1.73-4.232l-.039-.157zm26.478 3.557a.66.66 0 0 0-.072-.151l.002.003c-.174-.329-.596-.427-1.259-.29-2.066.426-2.866.164-3.156-.024a29.312 29.312 0 0 0 3.751-8.326l.049-.209c.339-1.313.997-4.403.153-5.913a1.982 1.982 0 0 0-.192-.298l.003.004a8.452 8.452 0 0 0-6.85-3.193h.011l-.141-.001c-1.342 0-2.633.22-3.838.625l.085-.025q-.321-.06-.645-.102a9.694 9.694 0 0 0-1.631-.16h-.008a6.406 6.406 0 0 0-3.835 1.065l.024-.015c-1.071-.401-5.984-2.056-9.025.098a6.868 6.868 0 0 0-2.324 6.432l-.006-.041a40.078 40.078 0 0 0 1.632 7.461l-.079-.284a23.079 23.079 0 0 0 1.852 4.6l-.061-.124a4.48 4.48 0 0 0 2.116 2.225l.026.012a2.062 2.062 0 0 0 2.317-.901l.005-.009a69.459 69.459 0 0 1 2.43-2.758 3.82 3.82 0 0 0 1.731.471h.006l.001.005q-.158.188-.309.382c-.424.538-.512.649-1.875.93-.388.08-1.418.292-1.433 1.014v.013c0 .147.043.284.117.399l-.002-.003c.312.395.751.678 1.254.788l.015.003c.399.13.859.205 1.335.205 1.1 0 2.106-.398 2.884-1.058l-.006.005a26.627 26.627 0 0 0 .459 6.526l-.027-.167a3.233 3.233 0 0 0 3.084 2.38h.002a5.012 5.012 0 0 0 1.07-.124l-.033.006a3.92 3.92 0 0 0 3.567-3.619l.001-.012c.188-1.088.502-3.593.673-5.125a.341.341 0 0 1 .072-.171l-.001.001c.001-.001.087-.059.534.038l.055.009.317.028.019.001a7.654 7.654 0 0 0 3.214-.557l-.051.018c.805-.373 2.256-1.29 1.993-2.087z" />
                </svg>

                <h4>PostgreSQL</h4>
              </span>
            </div>
          </span>

          {/** App Development Skills*/}
          <span className="bg-[#0F172A] px-4 pt-4 pb-6 rounded-2xl">
            <h3 className="text-center mb-4">App Development</h3>

            <div className="flex flex-col gap-10 items-center justify-center md:flex-row md:gap-6 text-black">
              <span className="flex items-center justify-center px-2 py-1 rounded-xl bg-white gap-2">
                <svg
                  className="fill-black w-10 h-10"
                  viewBox="-4 -3.2 38.40 38.40"
                >
                  <g id="SVGRepo_iconCarrier">
                    <path d="M13.8 4h4.5v2.2h1.2c.6 0 1 .4 1 1v4.6l-.6.8 6.2 10.8.9 3.9c.1.5-.5.9-.9.5l-2.9-2.7-2-3.5c-1.6.9-3.3 1.4-5.2 1.4s-3.6-.5-5.1-1.4l-2 3.5L6 27.8c-.4.4-1.1 0-.9-.5l.9-3.9 2.3-4.1c-1.5-1.7-2.3-4-2.3-6.4 0-.4 0-.8.1-1.1h3c-.1.4-.1.7-.1 1.1 0 1.3.4 2.5 1 3.6l2.3-3.9-.6-.8V7.2c0-.6.4-1 1-1h1.2V4zm.6 11.4-2 3.6c1.1.6 2.3 1 3.6 1s2.6-.4 3.6-1l-2.1-3.6-.8 1c-.4.5-1.2.5-1.6 0l-.7-1zm-.3-3.6c.4.7 1.1 1.1 1.9 1.1.8 0 1.6-.4 1.9-1.1.2-.3.3-.7.3-1.1 0-1.2-1-2.2-2.2-2.2s-2.2 1-2.2 2.2c0 .4.1.8.3 1.1z" />
                  </g>
                </svg>
                <h4>Android Studio</h4>
              </span>

              <span className="flex items-center justify-center px-2 py-1 rounded-xl bg-white gap-2">
                <svg className="fill-black w-10 h-10" viewBox="-4 -5 43 43">
                  <path d="m18.909 14.84-8.086 8.07 8.085 8.085h9.214l-8.073-8.083 8.073-8.073H18.91zm-.017-13.836L3.879 16l4.624 4.624L28.102 1.021h-9.194z" />
                </svg>

                <h4>Flutter</h4>
              </span>

              <span className="flex items-center justify-center px-2 py-1 rounded-xl bg-white gap-2">
                <svg className="fill-black w-10 h-10" viewBox="-2.5 -1.5 20 20">
                  <path d="M8.207.13c.26.075.458.286.517.55l1.199 5.397 5.396 1.2a.741.741 0 0 1 .364 1.247l-7.159 7.159a.741.741 0 0 1-.685.2L1.982 14.58a.741.741 0 0 1-.563-.563L.118 8.16a.741.741 0 0 1 .2-.685L7.475.317A.741.741 0 0 1 8.207.13Zm.283 6.332-.92-4.141-5.917 5.915.92 4.142L8.49 6.462Zm-4.868 6.964 4.142.92 5.915-5.915-4.141-.92-5.916 5.915Z" />
                </svg>

                <h4>Dart</h4>
              </span>

              <span className="flex items-center justify-center px-2 py-1 rounded-xl bg-white gap-2">
                <svg className="fill-black w-10 h-10" viewBox="-12 -12 55 55">
                  <path d="m1.734 32 15.068-15.333L32 32zM0 0h16L0 16.667zm17.865 0L0 18.667V32L32 0z" />
                </svg>

                <h4>Kotlin</h4>
              </span>
            </div>
          </span>
        </section>
      </div>

      {/** Projects Section */}
      <section
        id="projects"
        className="flex flex-col gap-8 px-8 pb-10 mt-[112px] bg-[#20283A] md:pt-4"
      >
        <h2 className="text-center">Projects</h2>

        <div className="grid grid-cols-3 gap-6">
          <a
            href="https://github.com/darajaw/Smart-Diary"
            className="z-1 p-6 bg-[#0F172A] hover:outline-1 rounded-md hover:bg-[#37475C] group"
          >
            <span className="flex flex-col items-center gap-3 md:gap-6">
              <Image
                width={350}
                height={450}
                className="rounded-md"
                src="/projects/diary.png"
                alt="Image of the Diary Website"
              />

              <div className="text-center">
                <h3 className="group-hover:text-green-300">Smart Diary</h3>
                <p>
                  A journaling website with user regristration and entry search
                  attached to an SQL database.
                </p>
              </div>

              <div className="flex gap-2 text-lg flex-wrap items-center justify-center">
                <span className="text-center bg-[#20283A] rounded-xl px-2"> SQL </span>
                <span className="text-center bg-[#20283A] rounded-xl px-2"> PHP </span>
                <span className="text-center bg-[#20283A] rounded-xl px-2"> HTML </span>
                <span className="text-center bg-[#20283A] rounded-xl px-2"> CSS </span>
                <span className="text-center bg-[#20283A] rounded-xl px-2"> JavaScript </span> 
                <span className="text-center bg-[#20283A] rounded-xl px-2"> Visual Studio Code </span>                
              </div> 
              

            </span>
          </a>

          <a
            href="https://devpost.com/software/screentime-buddy"
            className="p-6 bg-[#0F172A] hover:outline-1 rounded-md hover:bg-[#37475C] group"
          >
            <span className="flex flex-col items-center gap-3 md:gap-6">
              <Image
                width={150}
                height={100}
                className="rounded-md"
                src="/projects/screentime.jpg"
                alt="Image of the Pong game"
              />

              <div className=" text-center">
                <h3 className="group-hover:text-green-300">Screentime Buddy</h3>
                <p>
                  A conceptul app that shares phone usage data between users.
                </p>
              </div>
            
            <div className="flex gap-2 text-lg">
              <span className="text-center bg-[#20283A] rounded-xl px-2"> Android Studio </span>
              <span className="text-center bg-[#20283A] rounded-xl px-2"> Java </span>
              <span className="text-center bg-[#20283A] rounded-xl px-2"> XML </span>
              </div>    

            </span>
          </a>

          <a
            href="https://github.com/darajaw/JavaPong"
            className="p-6 bg-[#0F172A] hover:outline-1 rounded-md hover:bg-[#37475C] group"
          >
            <span className="flex flex-col items-center gap-3 md:gap-6">
              <Image
                width={300}
                height={350}
                className="rounded-md"
                src="/projects/pong.png"
                alt="Image of the Pong game"
              />

              <div className="text-center">
                <h3 className="group-hover:text-green-300">Pong Game</h3>
                <p>A Pong clone created using Java Swing and AWT libraries.</p>
              </div>

              <div className="flex gap-2 text-lg">
                <span className="text-center bg-[#20283A] rounded-xl px-2"> Java </span>
                <span className="text-center bg-[#20283A] rounded-xl px-2"> Java Swing </span>
              </div>  

            </span>
          </a>
        </div>
      </section>
    </main>
  );
}
